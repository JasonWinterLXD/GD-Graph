...
class MedicalGraph:
    def __init__(self):
        ...
        self.data_path = os.path.join(cur_dir, 'data_ready/data.json')# 获取json文件路径
        # self.g = Graph("http://localhost:7474", username="neo4j", password="wang1985")#老版本neo4j
        # self.g = Graph("http://localhost:7474", auth=("neo4j", "wangmeng"))
    ...
    def create_relationship(self, start_node, end_node, edges, rel_type, rel_name):
        with self.driver.session() as session:
            ...
            for edge in set(set_edges):
                ...
                q = edge[1]
                query = (
                    f"MATCH (p:{start_node} {{name: $p_name}}), (q:{end_node} {{name: $q_name}}) "
